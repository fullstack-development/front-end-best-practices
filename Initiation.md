...
- [1](#1) Создать README.md.
  ...
  - В проекте должно быть использовано по минимуму глобальных библиотек, то есть тех, код которых хранится в вашем репозитории, а не загружется через npm. Все они должны быть перечислены в Readme. Webpack, karma и все, что может быть поставлено через npm, точно не должно быть глобальным. 
