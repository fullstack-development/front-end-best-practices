...
1. [Общие требования](#general-requirements)
2. [Правила](#rules)
3. [Шрифты](#fonts)
4. [БЭМ](#bem)
5. [Возможные ошибки](#errors)

<a name="general-requirements"></a>

## Общие требования

<a name="general-requirements--pixel-perfect"></a>

- [1.1](#general-requirements--pixel-perfect) Главное! Используем [pixel-perfect](https://chrome.google.com/webstore/detail/perfectpixel-by-welldonec/dkaagdgjmgdmbnecmcefdhjekcoceebi?hl=ru), если заказчик предоставил макет.

<a name="general-requirements--no-external-styles"></a>

- [1.2](#general-requirements--no-external-styles) Не подключать стили с удаленных источников — только с наших серверов.

<a name="general-requirements--css"></a>

- [1.3](#general-requirements--css) CSS — опасная сила.
...
<a name="general-requirements--indent"></a>

- [1.4](#general-requirements--indent) Отступы делать пробелами, один уровень — 2 пробела.

<a name="general-requirements--quotes"></a>

- [1.5](#general-requirements--quotes) Использовать одинарные кавычки.

<a name="general-requirements--selectors"></a>

- [1.6](#general-requirements--selectors) Перед началом работы обязательно четко знать все приоритеты селекторов и все 4 вида возможных отношений.
...
<a name="general-requirements--imports"></a>

- [1.7](#general-requirements--imports) Не использовать `@import` внутри файлов.
...
<a name="general-requirements--rules-and-files"></a>

- [1.8](#general-requirements--rules-and-files) Все правила должны быть разбиты на разные файлы по страницам и блокам, каждый файл должен быть не длиннее, чем 1000 строк.

<a name="general-requirements--one-rule-one-line"></a>

- [1.9](#general-requirements--one-rule-one-line) Каждое CSS-правило должно быть на отдельной строке.
...
<a name="general-requirements--states"></a>

- [1.10](#general-requirements--states) Для всех интерактивных элементов (ссылки, кнопки, дропдауны, инпуты, селекты) дизайнер может прорисовать отдельные состояния и спрятать в ближайшем слое — всегда проверять для каждого элемента, есть ли отрисованное состояние.

<a name="general-requirements--design-errors"></a>

- [1.11](#general-requirements--design-errors) Дизайнеры могут ошибаться — иногда надо делать запросы на изменение макета, если они нарисовали что-то переусложненное.
...
<a name="general-requirements--adaptive"></a>

- [1.12](#general-requirements--adaptive) Пытаться по максимуму делать сайт резиновым (ставьте ширину в процентах и выставляйте max-width, min-width) — прибегайте к фиксированной ширине элемента, только если по макету от этого никак не уйти.
...
<a name="general-requirements--no-lepka"></a>

- [1.13](#general-requirements--no-lepka) При сборке проекта при отсутствии дизайна (на CSS-фреймворках, например) избегать "лепки".
...
<a name="general-requirements--styles-ordering"></a>

- [1.14](#general-requirements--styles-ordering) Все стили одного блока должны быть в одном месте и отсортированы по порядку их расположения на странице.
...
<a name="general-requirements--footet-to-bottom"></a>

- [1.15](#general-requirements--footet-to-bottom) Футер страницы должен быть всегда прижат к низу страницы, даже если на странице мало контента ([5 методов, как сделать это](http://dimox.name/press_footer_bottom_with_css/)).

<a name="general-requirements--names"></a>

- [1.16](#general-requirements--names) Имена.
...
<a name="rules--important"></a>

- [2.1](#rules--important) Не использовать `!important`.
...
<a name="rules--absolute"></a>

- [2.2](#rules--absolute) Не использовать `position: absolute` для стилизации, где это можно заменить другими подходами.

<a name="rules--relative"></a>

- [2.3](#rules--relative) Не двигать элемент при помощи `top`, `left`, `right` или `bottom` при `position: relative`, где это можно заменить другими подходами.
...
<a name="rules--negative-margin"></a>

- [2.4](#rules--negative-margin) Не использовать отрицательные margin.
...
<a name="rules--fixed-height"></a>

- [2.5](#rules--fixed-height) Избегать фиксирования высоты.
...
<a name="rules--fixed-align"></a>

- [2.6](#rules--fixed-align) Избегать фиксированных выравниваний.
...
<a name="rules--defaults"></a>

- [2.7](#rules--defaults) Прописывать дефолтный цвет фона, цвет шрифта, `font-size` и семейство шрифта у `html`.

  Далее размер шрифта будет ориентиром для всех элементов, у которых используется rem для единиц измерения кегля.

<a name="rules--visited"></a>

- [2.8](#rules--visited) При тестировании обязательно посмотреть цвет `:visited` ссылки.

  Браузер по умолчанию такие ссылки фиолетовыми делает, что чаще всего недопустимо в дизайне.

<a name="rules--cursor"></a>

- [2.9](#rules--cursor) Добавлять элементу `cursor: pointer`, если он кликабелен и по умолчанию этого не поддерживает.

<a name="rules--img"></a>

- [2.10](#rules--img) Тегу `<img>` стараться не выставлять одновременно `width` и `height` — тогда велика вероятность, что будут нарушены пропорции картинки. Если у картинки все-таки должны быть по макету четкие размеры, то сделать все через `<div>`, у которого картинка лежит на фоне и применить `background-size: cover`.

<a name="rules--unused-rules"></a>

- [2.11](#rules--unused-rules) Удалять все неиспользуемые правила и правила, которые никак не влияют на отображение (например, дублирующие свойства по умолчанию или нулевые `margin`, если они и так не выставлены) — они могут в дальнейшем при правках внести неочевидное поведение и из-за них тяжело разбираться в проекте.

<a name="rules--uppercase"></a>

- [2.12](#rules--uppercase) Слова, целиком состоящие из верхнего регистра, предпочтительно делать через стиль `text-transform: uppercase`, а не реальным вводом текста заглавными буквами.

<a name="rules--outline"></a>

- [2.13](#rules--outline) При установке элементу `outline: none` обязательно выставить стили для `:focus` этому элементу.
...
<a name="rules--media"></a>

- [2.14](#rules--media) Все использования `@media` должны идти сразу после основных правил элемента, для того чтобы можно было увидеть все правила и все возможные состояния элемента в одном экране без прокручивания.

<a name="rules--vendors"></a>

- [2.15](#rules--vendors) Вендорные префиксы должны идти перед общим стилем.
...
<a name="fonts--custom-fonts"></a>

- [3.1](#fonts--custom-fonts) Если клиент предоставил макеты с кастомными шрифтами, проверить, свободные ли они и, если нет, то запросить купленные файлы шрифтов.

<a name="fonts--support"></a>

- [3.2](#fonts--support) Если не требуется поддержка старых браузеров, шрифты должны быть в форматах `.woff2`, `.woff`. Сначала подключать `.woff2` шрифты, затем — `.woff` (пример ниже).

<a name="fonts--support"></a>

- [3.3](#fonts--support) Если в проекте должна быть поддержка нелатинских символов (например, русских), проверьте, что в файле шрифта эти символы есть.

<a name="fonts--style-weight"></a>

- [3.4](#fonts--style-weight) Разные начертания одного и того же шрифта подключайте под одним именем, но для разных `font-weight` и `font-style`.
...
<a name="fonts--fallback"></a>

- [3.5](#fonts--fallback) Всегда использовать как минимум один запасной шрифт и одно запасное семейство.
...
<a name="fonts--names-with-white-spaces"></a>

- [3.6](#fonts--names-with-white-spaces) Названия шрифтов, содержащие пробелы, цифры или знаки пунктуации, кроме дефисов, заключать в кавычки.
...
- [4.1](#bem--names) Именование БЭМ-сущностей должно следовать следующим ограничениям:
...
<a name="bem--forever"></a>

- [4.2](#bem--forever) Верстаем всегда по БЭМу, архитектура верстки должна быть компонентной.

  Каждый блок должен лежать в своей папке.

  Папки блоков нельзя вкладывать друг в друга.

<a name="bem--component-is-block"></a>

- [4.3](#bem--component-is-block) Каждый компонент — отдельный блок из методологии БЭМ.

<a name="bem--deps"></a>

- [4.4](#bem--deps) Каждый компонент должен быть иметь только явные зависимости, должен быть самодостаточен.
...
<a name="bem--customization"></a>

- [4.5](#bem--customization) Кастомизировать компоненты только через модификаторы, никаких примесей.
...
<a name="bem--positioning"></a>

- [4.6](#bem--positioning) Для позиционирования блока в родительском контейнере тоже не надо использовать миксы — решается данная проблема путем добавления специальных контейнеров.
...
<a name="errors--margin"></a>

- [5.1](#errors--margin) Верхний margin первого элемента смещает за собой всех родителей вниз.
...
<a name="errors--float"></a>

- [5.2](#errors--float) Родители игнорируют всех детей, у которых установлены `float: left` или `float: right`

<a name="errors--z-index"></a>

- [5.3](#errors--z-index) `z-index` свойство работает только для элементов, у которых значение position задано как `absolute`, `fixed` или `relative`.

<a name="errors--z-index-and-opacity"></a>

- [5.4](#errors--z-index-and-opacity) А еще на `z-index` может повлиять `opacity` Почитать про это можно [тут](https://habrahabr.ru/post/166435/).

<a name="errors--height"></a>

- [5.5](#errors--height) `height` в процентах не работает, если высота родителя формируется от контента.

  На `learn.javascript.ru` даже [статья](https://learn.javascript.ru/height-percent) есть.

<a name="errors--hover"></a>

- [5.6](#errors--hover) Все `:hover` эффекты проверять на мобильных устройствах внимательно в отдельном порядке, если проект поддерживает отображения на мобилах.
...
<a name="errors--pseudo-classes"></a>

- [5.7](#errors--pseudo-classes) Псевдоэлементы (`::after/before`) [не работают](https://stackoverflow.com/questions/14585070/css-after-pseudo-element-not-showing-up-on-img/14586588#14586588) с `self-closing tags` (`<img />`, `<input />`, etc.).

<a name="errors--hover-and-accessibility"></a>

- [5.8](#errors--hover-and-accessibility) Не использовать простой :hover для отображения новых элементов, которые нелегко достигнуть мышкой.
...
<a name="errors--non-flex-tags"></a>

- [5.9](#errors--non-flex-tags) В ряде браузеров (Chrome, Opera, Edge, Firefox 62-, Safari 10-) теги форм, такие как `button`, `fieldset` и `legend`, не могут стать flex-контейнерами.
