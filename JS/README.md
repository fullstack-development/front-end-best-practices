...
- [1.1](#good-practices--readable) Читаемость первостепенна
...
- [1.2](#good-practices--html-css-first) Все, что может быть написано на чистых html+css в рамках тех свойств, что дают заявленные браузеры - должно быть написано без применения JS.
...
- [1.3](#good-practices--critical-dependencies) Избегать неявных критических зависимостей.
...
- [1.4](#good-practices--airbnb) Знать и использовать стайлгайд от AirBnB.
...
- [1.5](#good-practices--eslint) Использовать `eslint` или аналоги.

<a name="good-practices--methods-naming"></a>

- [1.6](#good-practices--methods-naming) Методы.
...
- [1.7](#good-practices--lifetime-of-variables) Делайте время жизни переменных как можно короче.
...
- [1.8](#good-practices--submit-buttons) Все кнопки сабмита должны быть внутри `<form>` вместе со всеми соответсвующими инпутами. Обработку завершения заполнения вешать на событие `submit` формы, а не клик по кнопке.
...
- [1.9](#good-practices--conditions) Все проверки содержащие более одного условия должны быть вынесены.
...
- [1.10](#good-practices--assertions) Всегда избегать неявного приведения типов в JS.
...
- [1.11](#good-practices--prototypes) Не изменять прототипы стандартных конструкторов (например, `String.prototype` или `Function.prototype`), до тех пор пока вопрос внимательно не изучен и этот трюк не согласован со страшим разработчиком.

<a name="good-practices--constructor"></a>

- [1.12](#good-practices--constructor) Конструктор класса должен быть максимально легковесным.
...
- [1.13](#good-practices--handlers) Выносить обработчики событий в отдельные функции.
...
- [1.14](#good-practices--this) В обработчиках работать не с контекстом (this), а с аргументом объекта ивента, который был передан свыше.
...
- [1.15](#good-practices--array-methods) Стараться по максимуму избегать циклов и использовать встроенные методы массивов.
...
- [1.16](#good-practices--literals) Не использовать литералы из бизнес-логики напрямую - надо создавать объект с константами.
...
- [1.17](#good-practices--imports) Форматирование размещения импортов.
...
- [1.18](#good-practices--exports) Экспорты держать в конце файла.
...
- [1.20](#good-practices--global-variables) Избегать глобальных переменных.
...
- [1.21](#good-practices--global-variables-access) К глобальным переменным обращаться только как к свойствам window.
...
- [1.22](#good-practices--logging) Используйте специальные обертки для логирования, а не сырой `console.log`.
...
- [1.23](#good-practices--old-code) Старый код надо удалять, а не комментировать. Если он понадобится в дальнейшем, всегда можно будет посмотреть в истории коммитов.
...
- [1.25](#good-practices--browser-render-cycles) Очень полезно понимать, как работают циклы перерисовки браузера и как можно проверить и увеличить производительность работы с DOM - здесь в статье перечислены [полезные источники](https://isobar-us.github.io/code-standards/#javascript_javascript_performance).

<a name="good-practices--spell-checker"></a>

- [1.26](#good-practices--spell-checker) Код должен быть без орфографических ошибок. Имена переменных, функций, комментарии должны быть написаны правильно.
...
- [2.1](#good-practices--sense) Именовать переменные по смыслу.
...
- [2.2](#variable-naming--length) Старайтесь не давать переменным слишком длинные имена.
...
- [2.3](#variable-naming--case) При именовании переменных использовать `camelCase`, так как это принято в JavaScript комьюнити.
...
- [2.4](#variable-naming--boolean) Присваивайте булевым переменным имена, подразумевающие значение `true` или `false`.
...
- [2.5](#variable-naming--boolean-affirmative-form) Использовать утвердительную форму булевых переменных.
...
- [2.6](#variable-naming--explanatory-variable) Используйте объясняющие переменные.
...
- [2.7](#variable-naming--const) Имя константы должно характеризовать абстрактную сущность, представляемую константой, а не конкретное значение.
...
- [3.1](#function-naming--start-with-verbs) Все имена функций, за редкими исключениями (например, следование уже устоявшемуся соглашению в рамках какой-нибудь библиотеки), должны начинаться с глаголов.

<a name="function-naming--HOF"></a>

- [3.2](#function-naming--HOF) Функции высшего порядка, возвращающие функции, следует именовать по шаблону make + .\* + отглагольное существительное, где .\* — опциональный, синтаксически корректный набор слов уточняющий предназначение функции.

  Пример: `makeButtonClickHandler`.

<a name="function-naming--handlers"></a>

- [3.3](#function-naming--handlers) Обработчики событий.
...
- [4.1](#libaries--fork) Ни в коем случае никакой файл библиотеки не редактировать.
...
- [4.2](#libaries--best-practices) Изучайте best-practices для крупных библиотек, которые вы используете в проекте.
...
- [4.3](#libaries--node-modules) Модули должны использоваться из node_modules.
...
- [4.4](#libaries--right-choice) Выбор библиотеки.
...
- [5.1](#jquery--caching) Все классы, которые используем для поиска по DOM-у должны начинаться с префикса `js-`.
...
- [5.2](#jquery--selector-concatination) Конкатенировать селекторы, если с элементами будет проводиться одна и та же работа.
...
- [5.3](#jquery--naming-convention) Все переменные, которые поддерживают jQuery API, должны начинаться со знака.
...
- [5.4](#jquery--handlers-naming) Обработчики событий всегда именовать.
...
- [5.5](#jquery--keep-modularity) Поддерживать модульность.
...
- [6.1](#react--components) Все компоненты должны быть по умолчанию функциональными, то есть stateless ([что это такое?](https://reactjs.org/docs/components-and-props.html#stateless-functions)), и только при необходимости становиться `class-based` component.

<a name="react--render"></a>

- [6.2](#react--render) Метод `render()` должен быть чистым ([читаем доки](https://reactjs.org/docs/react-component.html#render)).

<a name="react--render-methods"></a>

- [6.3](#react--render-methods) Избегайте создания методов-рендеров в компонентах-классах (речь не о методе render, а о методах, которые вы можете написать помимо него, и которые тоже возвращают JSX).
...
- [6.4](#react--methods-orders) Метод `render()` и все методы, возвращающие куски DOM-a, должны располагаться в конце компоненты, сам метод `render` самым последним из публичных.

<a name="react--redux"></a>

- [6.5](#react--redux) Принципы работы с `action creators (redux)`.
...
- [7.1](#perfomance--caching) В проектах на jquery или чем-либо, где поиск DOM-элементов идет по селекторам - надо кэшировать все найденные элементы.
...
- [7.2](#perfomance--DOM) Все операции с DOM-ом лучше сначала проводить на виртуальных элементах, а потом сразу целиком вставлять в документ страницы.
...
- [7.3](#perfomance--complex-selectors) Избегать сложных селекторов.
...
- [7.4](#perfomance--scripts) Все js-ки подключаем внизу страницы перед закрывающимся тегом `<body>`.

<a name="perfomance--resources"></a>

- [7.5](#perfomance--resources) Полезные материалы:
...
- [8.1](#architecture--caching) Модульность - наше все!
...
- [8.2](#architecture--layers) В рамках модуля четко делить на слои и не мешать логику работы с DOM и работу с данными (например, ajax-запросы).
...
- [8.3](#architecture--mutations) Избегать мутаций одной переменной сразу в нескольких функциях.
...
- [8.4](#architecture--access-modifiers) В js нет публичных и частных свойств, поэтому частные методы просто делать с префиксами `"_"` (нижнее подчеркивание).
...
- [8.5](#architecture--methods-odrer) При написании класса все публичные методы объявляйте первыми, причем самым первым должен идти конструктор, все приватные методы группируйте по смыслу внизу класса
