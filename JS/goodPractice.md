...
4. **Методы**;
    ...
    *Именование:*
      * Имя метода должно быть декларативным (из названия должно быть понятно для чего нужен метод и что он делает не задумываясь о его внутренней реализации).
    
    *Параметры:*
      * Если в метод необходимо передать много параметров, объединяйте их в один объект. Например у вас есть метод который рисует прямоугольник принимая координаты точек, ширину, высоту, цвет, толщину рамок и другие. В таком случае проще будет передать один объект ‘options’. Это снимет с нас обязанность держать в голове все параметры а также их порядок и позволит удобно сделать многие свойства с дефолтными значениями;
      * В случае объекта параметров для наглядности можно использовать деструктуризацию;
      * Если параметр опциональный, необходимо ему задать значение по умолчанию.

    *Тело:*
      * Соблюдать принцип Single Responsibility. Метод должен выполнять только одну задачу (Например, метод `createReport`, который создает отчет и отправляет его на сервер необходимо разделить на два);
      * Метод должен быть компактным. Обычно, длина метода варьируется в пределах 10-40 строк. Это не значит что ваш метод обязательно должен должен быть в таких рамках, но если ваш метод занимает 200 строк, это повод задуматься можно ли его разделить на несколько отдельных методов. Про компактность методов можно почитать у С. Макконела “Совершенный код” и Р. Мартина “Чистый код”. Так же [здесь](https://softwareengineering.stackexchange.com/questions/133404/what-is-the-ideal-length-of-a-method-for-you) Неплохое рассуждение о размерах метода.

5. **Делайте время жизни переменных как можно короче**;
    > Объявление переменной, должно быть рядом с ее использованием. Это дает более быстрое понимание того, что происходит в коде, плюс снижается вероятность неправильного использования переменной или ее перезапись.  
    > Более подробнее про время жизни переменной можно прочитать у С. Макконнелла в "Совершенном коде".

6. **Все кнопки сабмита должны быть внутри `<form>` вместе со всеми соответсвующими инпутами и обработку завершения заполнения вешать на событие `submit` формы, а не клик по кнопке**;
...
9. **Не изменять прототипы стандартных конструкторов (String.prototype или Function.prototype), до тех пор пока вопрос внимательно не изучен и этот трюк не согласован со страшим разработчиком**;
...
11. **Выносить обработчики событий в отдельные функции**;
    ...
    >
    >    bindEvents () {
    >      elem.addEventListener( "click" , this.greet);
    >    }
    >
    >    greet () {
    >      // это самодостаточная функция, которую мы можем в других местах легко использовать, у нее есть имя, так что легче будет понять ее назначение и смысл.
    >      alert('Спасибо!');
    >    }
    >  }
    >```
...
12. **В обработчиках работать не с контекстом (this), а с аргументом объекта ивента, который был передан свыше**;
    ...
    > Хорошо:
    >```javascript
    >  $(this).click(function(event) {  // не называйте просто "e", пишите "event"
    >    let $buttonWidth = $(event.currentTarget).width();
    >  })
    >```
...
19. **К глобальным переменным обращаться только как к свойствам window**;
    ...
    >  function test () {
    >    window.foo = 'hello world';  // обратить внимание, что переменная создана без var, значит она глобальная
    >  }
    >  test();
...
22. **Всегда следует добавлять и удалять классы у DOM-элементов, а не изменять конкретные свойства css у элементов**;
...
23. **Очень полезно понимать, как работают циклы перерисовки браузера и как можно проверить и увеличить производительность работы с DOM - здесь в статье перечислены [полезные источники](https://isobar-idev.github.io/code-standards/#javascript_javascript_performance)**;
