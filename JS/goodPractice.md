...
4. **Методы**;
    ...
    *Именование:*
      * Имя метода должно быть `self description`(Описывать сам себя). Из названия должно быть понятно для чего нужен метод и что он делает. Не должно быть причин задумываться о его внутренней реализации.
